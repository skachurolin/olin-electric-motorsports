...
@click.command()
@click.option('-s', '--source', prompt='path to symbol library', help='Symbol library', type=click.File())
...
def library2symbols(source, out):
    ...
    while True:
        ...
        if (line[0] == '('): # start of symbol
            ...
            file_name = out + '/' + symbol_name + (".derived-symbol" if is_derived else ".symbol")
